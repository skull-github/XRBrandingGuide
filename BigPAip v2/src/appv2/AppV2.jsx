    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 32 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 8, textAlign: 'left', justifyContent: 'center' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 500, color: '#bfa76a', margin: 0 }}>
...
function AppV2() {
  ...
  try {
    return (
      <div className="App" style={{ padding: '2.5rem', maxWidth: 1200, margin: '0 auto', alignItems: 'flex-start', justifyContent: 'flex-start' }}>
        <h1 style={{ fontSize: 32, fontWeight: 800, color: '#222', margin: '0 0 2.5rem 0', letterSpacing: 1, textAlign: 'center', width: '100%' }}>MLB Team Branding & Roster Explorer</h1>
        ...
        {teams.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 40, alignItems: 'center', justifyContent: 'center', width: '100%', maxWidth: 900, marginLeft: 'auto', marginRight: 'auto' }}>
            ...
            {/* AL and NL All-Star anchor buttons using TeamSpotWithColor */}
            <a
              href="#al-allstars"
              style={{ textDecoration: 'none', border: 'none', background: 'none', cursor: 'pointer', padding: 0, display: 'inline-block' }}
              title="American League All-Stars"
            >
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#003263', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px #0003', fontWeight: 700, color: '#fff', fontSize: 18 }}>
                AL
              </div>
            </a>
            <a
              href="#nl-allstars"
              style={{ textDecoration: 'none', border: 'none', background: 'none', cursor: 'pointer', padding: 0, display: 'inline-block' }}
              title="National League All-Stars"
            >
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#c41e3a', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 2px 8px #0003', fontWeight: 700, color: '#fff', fontSize: 18 }}>
                NL
              </div>
            </a>
          </div>
...
function MLBTVGallery({ mlbtv }) {
  ...
  const assets = [
    mlbtv.defaultBackground?.imageUrl && { label: 'Default Background', url: mlbtv.defaultBackground.imageUrl },
    mlbtv.welcome?.browse?.imageUrl && { label: 'Welcome Browse', url: mlbtv.welcome.browse.imageUrl },
    mlbtv.welcome?.create?.imageUrl && { label: 'Welcome Create', url: mlbtv.welcome.create.imageUrl },
    mlbtv.welcome?.explore?.imageUrl && { label: 'Welcome Explore', url: mlbtv.welcome.explore.imageUrl },
  ].filter(Boolean);
  function handleCopy() {
    navigator.clipboard.writeText(modal.src);
    setCopied(true);
  ...
  return (
    <>
      <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap', alignItems: 'flex-start', marginBottom: 8 }}>
        {assets.map(asset => (
          <div key={asset.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 180 }}>
            <img
              src={asset.url}
              alt={asset.label}
              style={{ maxWidth: 180, width: '100%', height: 'auto', borderRadius: 6, background: '#eee', marginBottom: 0, objectFit: 'contain', cursor: 'pointer' }}
          ...
          return (
            <div key={asset.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer', width: 120 }}>
              ...
              <img
                ...
                onClick={() => setModal({ open: true, src: asset.url, alt: asset.label })}
              />
              <span style={{ fontSize: 13, color: '#bfa76a', marginTop: 2, letterSpacing: 0.5 }}>BG: {bgColor}</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 28, minHeight: 80, marginBottom: 18, width: '100%', zIndex: 2 }}>
        ...
        />
        <div>
          <h1 style={{ fontWeight: 800, fontSize: 28, color: '#1a1a1a', marginBottom: 6, letterSpacing: 1, textShadow: '0 1px 0 #fff8' }}>{team.clubFullName}</h1>
          <div style={{ fontSize: 15, color: '#bfa76a', marginTop: 2 }}>
      ...
      {revealed && (
        <div style={{ marginTop: 18, width: '100%' }}>
          {/* Roster Section */}
          <h2 style={{ fontWeight: 800, fontSize: 24, margin: '1.5rem 0 0.5rem 0', color: '#4fd1c5', letterSpacing: 1, textAlign: 'left' }}>Roster</h2>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 40, flexWrap: 'wrap', marginBottom: 40 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
              <WordmarkOverlay
                src={`https://img.mlbstatic.com/mlb-photos/image/upload/w_512,f_png,q_auto/v1/team/${team.teamID}/logo/cap/current`}
                alt="Team Logo Default"
                url={`https://img.mlbstatic.com/mlb-photos/image/upload/w_512,f_png,q_auto/v1/team/${team.teamID}/logo/cap/current`}
                title={null}
                enableModal={true}
              />
              <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 100, whiteSpace: 'normal', overflow: 'visible', textOverflow: 'unset', boxSizing: 'border-box', paddingLeft: 10 }} title="Team Logo Default">Team Logo Default</div>
            </div>
            {/* League Logo Spot */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
              <WordmarkOverlay
                src={`https://img.mlbstatic.com/mlb-photos/image/upload/w_512,f_png,q_auto/v1/team/${team.teamID}/logo/spot/current`}
                alt="League Logo Spot"
                url={`https://img.mlbstatic.com/mlb-photos/image/upload/w_512,f_png,q_auto/v1/team/${team.teamID}/logo/spot/current`}
                title={null}
                enableModal={true}
              />
              <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 100, whiteSpace: 'normal', overflow: 'visible', textOverflow: 'unset', boxSizing: 'border-box', paddingLeft: 10 }} title="League Logo Spot">League Logo Spot</div>
            </div>
            {/* Team Logo Spot */}
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
              <WordmarkOverlay
              ...
              {team.capLogos?.rasterDarkActive && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
                  <WordmarkOverlay
                    ...
                    enableModal={true}
                  />
                  <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 100, whiteSpace: 'normal', overflow: 'visible', textOverflow: 'unset', boxSizing: 'border-box', paddingLeft: 10 }} title="Primary Logo">Primary Logo</div>
                </div>
              )}
              {/* Alternate Logo 1 */}
              {team.capLogos?.rasterLightActive && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
                  <WordmarkOverlay
                    ...
                    alt="Alternate Logo 1"
                    url={team.capLogos.rasterLightActive.replace('{type}', 'png').replace('{width}', '100')}
                    title={null}
                    enableModal={true}
                  />
                  <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 100, whiteSpace: 'normal', overflow: 'visible', textOverflow: 'unset', boxSizing: 'border-box', paddingLeft: 10 }} title="Alternate Logo 1">Alternate Logo 1</div>
                </div>
              )}
              {/* Alternate Logo 2 */}
              {team.capLogos?.svgDarkActive && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
                  <WordmarkOverlay
                    ...
                    alt="Alternate Logo 2"
                    url={team.capLogos.svgDarkActive.replace('{type}', 'png').replace('{width}', '100')}
                    title={null}
                    enableModal={true}
                  />
                  <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 100, whiteSpace: 'normal', overflow: 'visible', textOverflow: 'unset', boxSizing: 'border-box', paddingLeft: 10 }} title="Alternate Logo 2">Alternate Logo 2</div>
                </div>
              )}
              {/* App Icon */}
              {team.capLogos?.svgLightActive && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
                  <WordmarkOverlay
                    ...
                    enableModal={true}
                  />
                  <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 100, whiteSpace: 'normal', overflow: 'visible', textOverflow: 'unset', boxSizing: 'border-box', paddingLeft: 10 }} title="App Icon">App Icon</div>
                </div>
              )}
            </div>
              ...
              {team.wordmarks?.rasterLight && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
                  <WordmarkOverlay
                    ...
                    enableModal={true}
                  />
                  <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 120, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', boxSizing: 'border-box', paddingLeft: 10 }} title="Wordmark Light">Wordmark Light</div>
                </div>
              )}
              {/* Wordmark Dark */}
              {team.wordmarks?.rasterDark && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: 120 }}>
                  <WordmarkOverlay
                    ...
                    enableModal={true}
                  />
                  <div style={{ textAlign: 'left', fontSize: 13, color: '#888', marginTop: 10, width: 120, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', boxSizing: 'border-box', paddingLeft: 10 }} title="Wordmark Dark">Wordmark Dark</div>
                </div>
              )}
            </div>
          </div>
...
function BigpapiColorSwatch({ teamID }) {
  ...
  return (
    <div className="branding-color" style={{ position: 'relative', width: 100 }}>
      <div
        className="branding-color-swatch"
        style={{ background: hex, cursor: 'pointer', width: 100, height: 48, borderTopLeftRadius: 8, borderTopRightRadius: 8, borderBottomLeftRadius: 0, borderBottomRightRadius: 0, border: '1.5px solid #222', boxSizing: 'border-box' }}
        title="Click to copy hex"
        onClick={() => {
          navigator.clipboard.writeText(hex);
        }}
      ></div>
      <div className="branding-color-label" style={{ width: 100, background: '#18181b', borderBottomLeftRadius: 8, borderBottomRightRadius: 8, boxSizing: 'border-box', textAlign: 'center', padding: '6px 0' }}>
...
function WordmarkOverlay({ src, alt, url, title, enableModal }) {
